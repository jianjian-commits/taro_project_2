...
@observer
class PurchaserProportion extends React.Component {
  contentComponent = () => {
    const { purchaserProportion, purchaseFilter, isFullScreen } = Store
      ...
      <Box hasGap style={UiStyle.getModalBackgroundColor(isFullScreen)}>
        ...
        <div>
          <PieECharts
            ...
            data={purchaserProportion.slice()}
            axisGroup={[
              { itemName: 'purchaser_name', value: 'purchase_sum_money' },
            ]}
            hasNoData={purchaserProportion.length === 0}
            axisGroupName={['采购员占比']}
            radiusList={[40, 70]}
...
export default PurchaserProportion
