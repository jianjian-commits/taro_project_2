...
class Component extends React.Component {
  ...
  handleSavePrioritySupplier = (index, supplier) => {
    ...
    store.setAllTypeByBatch(params, updateOne).then((json) => {
  ...
  render() {
    const {
      error: { error_list, error_num, total_num },
    } = supplierStore
    const { sku } = this.props.location.query
    return (
      <BoxTable
        info={
          <BoxTable.Info>
            {i18next.t('设置商品优先供应商失败列表')}
          </BoxTable.Info>
        }
      >
        <div>
