...
class TemList extends React.Component {
    ...
    this.columns = [
      ...
      { Header: i18next.t('模板名称'), accessor: 'content.name' },
      {
        ...
        Cell: (cellProps) => {
          ...
          return isKidTemp(original?.category)
            ? t('账户配送单')
            : t('商户配送单')
        },
      },
        ...
        Cell: (cellProps) => {
          ...
          return (
            <div className='b-order-printer-hover-wrap'>
              {canEdit && (
                <a
                  onClick={this.handleSetDefault.bind(this, index, category)}
      ...
      { Header: i18next.t('创建人'), accessor: 'creator' },
      {
        ...
        // 根据category决定去商户详情页还是账户详情页
        Cell: (cellProps) => (
          <OperationCell>
            <OperationDetail
              href={`#${
                isKidTemp(cellProps.original?.category)
  ...
  handleSetDefault = async (index, category) => {
    const { temListStore } = this.props
    this.setState({ loading: true })
    try {
      await temListStore.setDefaultTem(index, category)
  ...
  handleDel = (id, category) => {
    const { temListStore } = this.props
    temListStore.deleteTem(id, category).then(() => {
